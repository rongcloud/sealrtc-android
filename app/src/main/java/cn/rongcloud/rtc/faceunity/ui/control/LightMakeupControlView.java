...
import cn.rongcloud.rtc.faceunity.entity.LightMakeupEnum;
import cn.rongcloud.rtc.faceunity.entity.LightMakeupItem;
import cn.rongcloud.rtc.faceunity.ui.adapter.BaseRecyclerAdapter;
import cn.rongcloud.rtc.faceunity.ui.adapter.SpaceItemDecoration;
...
public class LightMakeupControlView extends FrameLayout {
    ...
    private boolean mEnableLightMakeup;
        ...
        mDiscreteSeekBar.setOnProgressChangeListener(
                new DiscreteSeekBar.OnProgressChangeListener() {
                    ...
                    public void onProgressChanged(
                        ...
                        LightMakeupCombination lightMakeupCombination =
                                mLightMakeupAdapter.getSelectedItems().valueAt(0);
                        ...
                        if (makeupItems != null) {
                            for (LightMakeupItem makeupItem : makeupItems) {
                                float fixLevel =
                                        makeupItem.getDefaultLevel()
                                                * level
                                                / LightMakeupEnum.LIGHT_MAKEUP_OVERALL_INTENSITIES
                                                        .get(nameId);
    ...
    public void selectDefault() {
        ...
        mFirstSetup = true;
        mDiscreteSeekBar.post(
                new Runnable() {
                    @Override
                    public void run() {
                        mLightMakeupAdapter
                                .getOnItemClickListener()
                                .onItemClick(mLightMakeupAdapter, null, 1);
                    }
                });
    }
    ...
    private class LightMakeupAdapter extends BaseRecyclerAdapter<LightMakeupCombination> {
        ...
        protected void bindViewHolder(BaseViewHolder viewHolder, LightMakeupCombination item) {
            viewHolder
                    .setText(R.id.tv_makeup, getResources().getString(item.getNameId()))
                    .setImageResource(R.id.iv_makeup, item.getIconId());
        }

        @Override
        protected void handleSelectedState(
                BaseViewHolder viewHolder, LightMakeupCombination data, boolean selected) {
            ((TextView) viewHolder.getViewById(R.id.tv_makeup))
                    .setTextColor(
                            selected
                                    ? getResources().getColor(R.color.main_color)
                                    : getResources().getColor(R.color.colorWhite));
            viewHolder.setBackground(
                    R.id.iv_makeup, selected ? R.drawable.control_filter_select : 0);
        }
    }

    public boolean isEnableLightMakeup() {
        return mEnableLightMakeup;
    }

    private class OnLightMakeupClickListener
            implements BaseRecyclerAdapter.OnItemClickListener<LightMakeupCombination> {

        @Override
        public void onItemClick(
                BaseRecyclerAdapter<LightMakeupCombination> adapter, View view, int position) {
            ...
            if (position == 0) {
                ...
                if (makeupItems != null) {
                    for (LightMakeupItem makeupItem : makeupItems) {
                        float fixLevel =
                                makeupItem.getDefaultLevel()
                                        * level
                                        / LightMakeupEnum.LIGHT_MAKEUP_OVERALL_INTENSITIES.get(
                                                nameId);
                        makeupItem.setLevel(fixLevel);
                        mOnFUControlListener.onLightMakeupItemLevelChanged(makeupItem);
                    }
                ...
                Pair<Filter, Float> filterFloatPair =
                        LightMakeupEnum.LIGHT_MAKEUP_FILTERS.get(nameId);
