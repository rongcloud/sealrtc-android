...
//        },UVCCamera.PIXEL_FORMAT_NV21);
    ...
    private void createTexture() throws Exception{
        try {
            ...
            // Both these statements have been observed to fail on rare occasions, see BUG=RongRTC:5682.
        ...
        if (myDevices != null && myDevices.size() != 0) {
            ...
            if (myDevices.size() > 1) {
                for (UsbDevice usbDevice : myDevices) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && usbDevice.getProductName().contains("Camera")) {
    ...
    protected abstract void onTextureFrameAvailable(int oesTextureId, int width, int height, float[] transformMatrix, long timestampNs);
    ...
    public Size getClosestSupportedSize(
        ...
        Size size = Collections.min(supportedSizes, new ClosestComparator<Size>() {
            @Override
            int diff(Size size) {
                //First time, try to select a Size whose width and length are not shorter than the requested.
                //If no Size is suitable, then select the biggest Size which is closest with the requested.
                if (size.width - requestedWidth >= 0 && size.height - requestedHeight >= 0)
                    return size.width - requestedWidth + size.height - requestedHeight;
                else
