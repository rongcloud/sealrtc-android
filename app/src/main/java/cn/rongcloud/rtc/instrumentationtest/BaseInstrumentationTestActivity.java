...
public class BaseInstrumentationTestActivity extends RongRTCBaseActivity implements OnClickListener, OnClickItemChildListener<OperationModel, OperationViewHolder> {
    ...
    void onJoinRoom() {
        room = RTCEngineImpl.getInstance().getRoom();
        if (room != null) {
            onJoinRoomSuccess();
            return;
        }
        ...
        RTCEngineImpl.getInstance().joinRoom(mEvRoomId.getText().toString(), roomType, new RTCResultDataCallbackWrapper<RCRTCRoom>(this) {
            @Override
    ...
    protected void initRTC() {
        RTCEngineImpl.getInstance().init(getApplication(), getRTCConfig());
        RTCEngineImpl.getInstance().enableSpeaker(false);
    }
...
//        mImLayout.setVisibility(View.GONE);
    ...
    private void leaveRoom() {
        RTCEngineImpl.getInstance().leaveRoom(null);
    }
